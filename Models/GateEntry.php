...
class GateEntry extends Model
{
  ...
  function saveCommonData($data){
    ...
    $data['orig_id'] = !empty($data['orig_id']) ? $data['orig_id'] : 0;	

    $data['loaded_weight'] = !empty($data['loaded_weight']) ? $data['loaded_weight'] : 0.0;	

    $data['deduct_packing_material'] = !empty($data['deduct_packing_material']) ? $data['deduct_packing_material'] : 0;	

    // $data['second_weight_date'] = !empty($data['second_weight_date']) ? $data['second_weight_date'] : '0000-00-00 00:00:00';	
		$this->fill($data)->save();
	...
	function checkSimilarEntries($data){
	  ...
	  $deductPackMat = '';
		...
		if(isset($data['chatni_report_no']))
			$obj->where('chatni_report_no', $data['chatni_report_no']);
		...
		$obj->where('first_weight_date','>=', date('Y-m-d 00:00:00', strtotime("-1 Day")));
	...
	function differences(){
		return $this->hasMany(GateEntryDiff::class,'gate_entry_id');
	}
	...
	function saveQualityCuts($qualityCuts){
		...
		foreach($qualityCuts as $qualityCut){
			if(!$qualityCut['bags'])
			 ...
			 $this->qualityCuts()->create($qualityCut);
		}
	}
